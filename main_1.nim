...
text = replace(text, "е", "ё") # Замена букв "е" на буквы "ё"
var sentences = split(text, ". ") # Разделение текста на отдельные предложения
sentences = map(sentences, proc(item: string): string = reversed(item)) # map возвращает новую последовательность с результатом применения операции (reversed — разворот строки) к каждому элементу в указанной последовательности
sentences = map(sentences, proc(item: string): string = item.replace(".", "")) # Разделение текста было произведено по точке с пробелом, но последнее предложение не имеет пробела после точки.
text = "" 
apply(sentences, proc(item: string) = text.add("." & item & "\n")) # Добавляем в переменную text каждое предложение, возвращая точки и ставя красные строки. apply используется вместо map, т.к. производится операция, не возвращающая ничего
text.removeSuffix("\n") # удаляем последний символ новой строки. removeSuffix удаляет указанное вхождение с конца строки

# Прим. 
# Из библиотеки strutils: replace, split, removeSuffix
# Из библиотеки sequtils: map, apply
# Из библиотеки unicode: reversed, add
